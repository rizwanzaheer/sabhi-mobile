import { normaliseClaimTree, isTopLevelSingleKey, renderCrendentialItem } from '../claim'
...
describe('Parse claims', () => {
  it('should return true for a topLevel single key claim', () => {
    const isTopLevel = isTopLevelSingleKey(verificationWithSinglekey, 0)
    expect(isTopLevel).toBeTruthy()
  })

  it('should return the string', () => {
    const item = renderCrendentialItem(credentialItems.string)
    expect(item).toEqual(credentialItems.string)
  })

  it('should convert true to Yes', () => {
    const item = renderCrendentialItem(credentialItems.truthy)
    expect(item).toEqual('Yes')
  })

  it('should convert false to No', () => {
    const item = renderCrendentialItem(credentialItems.falsey)
    expect(item).toEqual('No')
  })

  it('should return null for an image url', () => {
    const item = renderCrendentialItem(credentialItems.image)
    expect(item).toEqual(null)
  })

  it('should return input for url', () => {
    const item = renderCrendentialItem(credentialItems.url)
    expect(item).toEqual(credentialItems.url)
  })

  it('should normailse a simple claim', () => {
    const normalised = normaliseClaimTree(simpleVerification.claim)
  ...
  it('should normailse a complex claim', () => {
    const normalised = normaliseClaimTree(complexVerification.claim)
